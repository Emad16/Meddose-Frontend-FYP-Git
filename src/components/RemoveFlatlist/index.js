...
const RemoveFLatlist = forwardRef(({filter}, ref) => {
  ...
  const getRemovedList = async () => {
    try {
      showLog(page, 'Page number in getRemovedList');
      setLoader(true);
      const response = await axios.get(
        `${apiBaseURLV1}/get/listings/${page}?own=1&status=draft`,
        {
          headers: {
